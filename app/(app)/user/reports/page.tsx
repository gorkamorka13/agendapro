...
export default function UserReportsPage() {
  ...
  const handleExportPDF = async (options: ExportOptions) => {
    ...
    if (options.detailedLogs && reportData.workedHours.length > 0) {
      ...
      autoTable(doc, {
        startY: currentY + 5,
        head: [['Date', 'Patient', 'Début', 'Fin', 'Durée']],
    ...
    doc.save(`Rapport_${session.user.name || 'User'}_${startDate}.pdf`);
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <table className="w-full text-left text-sm">
                <thead className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider border-b border-slate-100 dark:border-slate-700">
                  <tr>
                    ...
                    <th className="pb-3 px-2">Heure début</th>
                    <th className="pb-3 px-2">Heure fin</th>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700">
              <div className="flex items-center gap-3 mb-2 text-slate-500 dark:text-slate-400">
                ...
                <span className="text-sm font-medium">Mon Taux Horaire</span>
              </div>
            ...
            <div className="bg-blue-600 p-5 rounded-2xl shadow-lg shadow-blue-100 dark:shadow-none text-white">
              <div className="flex items-center gap-3 mb-2 opacity-80">
                <TrendingUp size={18} />
                <span className="text-sm font-medium">Ma Paie Estimée</span>
              </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 flex flex-col col-span-3">
              ...
              <div className="flex-1 overflow-y-auto max-h-[400px] space-y-3 pr-2 scrollbar-thin scrollbar-thumb-slate-200 dark:scrollbar-thumb-slate-700">
                  ...
                  <div key={i} className="p-3 bg-slate-50 dark:bg-slate-900 p-3 rounded-xl border border-slate-100 dark:border-slate-700 flex justify-between items-center">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="text-sm font-bold text-slate-700 dark:text-slate-200">{entry.patient}</span>
                      </div>
                    ...
                    <div className="text-right">
                      ...
                      <div className="text-xs font-medium text-slate-400 dark:text-slate-500">{entry.pay} €</div>
                    </div>
                  </div>
                ))}
              </div>
